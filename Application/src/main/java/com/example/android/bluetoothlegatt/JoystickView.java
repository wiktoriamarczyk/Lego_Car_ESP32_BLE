...
public class JoystickView extends View
    ...
    */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Integer x = mFixedCenterX;
        Integer y = mFixedCenterY;

        canvas.drawRect(x.floatValue() - 400, y.floatValue() - 400,mBackgroundRadius*2.5f, mBackgroundRadius*2.5f, mPaintBackground );
    ...
    public boolean onTouchEvent(MotionEvent event) {
       ...
       // }
        ...
        int maxDistance = (int)(mBackgroundRadius * 1.25);


        if ((mPosX - mCenterX) > maxDistance){
            mPosX = mCenterX + maxDistance;
        } else if ((mPosX - mCenterX) < -maxDistance){
            mPosX = mCenterX - maxDistance;
        }

        if ((mPosY - mCenterY) > maxDistance){
            mPosY = mCenterY + maxDistance;
        } else if ((mPosX - mCenterY) < -maxDistance){
            mPosY = mCenterY - maxDistance;
        }
